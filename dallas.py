#!/usr/bin/env python
"""
dallas.py 13.00 Dallas 1 wire interface to PrivateEyePi
---------------------------------------------------------------------------------
...
def GetLocationList():
        ...
        LocationList = GetDataFromHost(21,[0])
        if LocationList==False:
                return 1
            
        if globals.PrintToScreen: print LocationList
        
def GetLocationDesc(number):
        global LocationList
                        
        numrf = len(LocationList)
        
        for i in range(numrf):
                if LocationList[i][0]==number:
                        return(LocationList[i][1])
        return 0
...
def NotifyHostTemperature():
        ...
        for z in range(0,numtemp,1):
                ...
                if globals.LCDTemperature==True:
                        LocationDesc = GetLocationDesc(globals.DallasSensorNumbers[z])
                        ...
                        SendToLCD([globals.DallasSensorNumbers[z],LocationDesc,0,3,TempBuffer[0],0,globals.Farenheit,0,0])
